...
func (f *Flow) RunLive(ctx agent.InvocationContext) (agent.LiveSession, iter.Seq2[*session.Event, error], error) {
	...
	go func() {
		...
		liveConnectConfig := &genai.LiveConnectConfig{
			...
			// quran-agent patch: upstream drops these two LiveRunConfig fields, so VAD
			// tuning and context compression never reach the Live API.
			RealtimeInputConfig:      runCfg.Live.RealtimeInputConfig,
			ContextWindowCompression: runCfg.Live.ContextWindowCompression,
		}
		if ric := liveConnectConfig.RealtimeInputConfig; ric != nil && ric.AutomaticActivityDetection != nil {
			// quran-agent patch: one-line proof at connect time that VAD tuning is in the
			// setup message (upstream silently dropped it).
			silence := int32(-1)
			if ms := ric.AutomaticActivityDetection.SilenceDurationMs; ms != nil {
				silence = *ms
			}
			log.Printf("live connect: vad silence_ms=%d start=%q end=%q compression=%v",
				silence,
				ric.AutomaticActivityDetection.StartOfSpeechSensitivity,
				ric.AutomaticActivityDetection.EndOfSpeechSensitivity,
				liveConnectConfig.ContextWindowCompression != nil)
		}
